 ...
 * @since		Version 1.0
 */
class Sales_manager_mod extends CI_Model {
   ...
   /**
    ...
    function add(){
        ...
        $password                   =   $_POST['sales_manager_password'];
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
     ...
     * @return array
     */
    function sales_manager_update($id = NULL,$data_post=array()){   
           ...
           // pr($affected);die;
            if($affected>=0){
                ...
                $res['status'] ='success';
                $res['msg'] ='Sales Manager details updated successfully';
            } else {
                $res['status'] ='error';
                $res['msg'] ='Sales Manager details not updated successfully';
            }
     ...
     * @return array
     */
    function sales_manager_list_ajax($filter){
       ...
       $this->db->select("SQL_CALC_FOUND_ROWS u.id,concat(u.first_name,' ',u.last_name) as `sales_manager_name`,u.email,u.mobile_number,u.status",False);
       $this->db->from(" $this->tbl_users as u");
        
        $this->db->where('u.user_type','4');
        $columns = array('u.id','u.id','u.first_name','u.last_name','u.email','u.mobile_number','u.status');
...
//---------------------------------------------------------------------------------------------------------------------
    function sales_manager_mobile_exist($valuekey)
    {
