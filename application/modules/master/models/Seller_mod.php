...
//THIS FUNCTION ADD COLOR1
    public function add($data) {               
        if ($this->db->insert("aa_seller", $data)) {
            return true;
        }
    }
...
//  THIS FUNCTION DELETE city DATA
    function delete_city($id) {
        ...
        $this->db->update('aa_seller', $data);
        return true;
    }

//  THIS FUNCTION EDIT city DATA
    function edit($id) {
        $data['seller_id'] = $id;//$this->input->post('seller_name');
        $postdata = array(
        ...
        $this->db->where('seller_id', $id);
        $this->db->update('aa_seller', $postdata);
    }
...
//  THIS FUNCTION VIEW city DATA
    function view($id) {
        $this->db->select('*');
        $this->db->where('seller_id', $id);
        return $query = $this->db->get("aa_seller")->row();
    }
    ...
    function count_city_data() {
        ...
        return $query = $this->db->get('aa_seller');
    }
    ...
    function get_city_data($parent_id = "") {        
        ...
        $this->db->select('wct.*, wct.seller_id,wct.name as city_name,wct.status, wcntry.name as country_name,wcntry.seller_id');
        $this->db->from('aa_seller as wct');
         $this->db->join('aa_seller wcntry','wct.seller_id=wcntry.seller_id','left');
        ...
        if (@$requestData['order'][0]['column'] && @$requestData['order'][0]['dir']) {
            ...
            $this->db->order_by("wct.seller_id", "desc");
        }
