...
class Reason_mod extends CI_Model {
...
//THIS FUNCTION ADD COLOR1
    public function add($data) {               
        if ($this->db->insert("aa_bank_reason", $data)) {
            return true;
        }
    }
...
//  THIS FUNCTION DELETE city DATA
    function delete_city($id) {
        ...
        $this->db->update('aa_bank_reason', $data);
        return true;
    }

//  THIS FUNCTION EDIT city DATA
    function edit($id) {
        $data['quality_id'] = $id;//$this->input->post('quality_name');
        $data['name'] = $this->input->post('quality_name');
...
//  THIS FUNCTION VIEW city DATA
    function view($id) {
        $this->db->select('*');
        $this->db->where('quality_id', $id);
        return $query = $this->db->get("aa_bank_reason")->row();
    }
    ...
    function count_city_data() {
        ...
        return $query = $this->db->get('aa_bank_reason');
    }
    ...
    function get_city_data($parent_id = "") {        
        ...
        $this->db->select('wct.quality_id,wct.name as city_name,wct.status, wcntry.name as country_name,wcntry.quality_id');
        $this->db->from('aa_bank_reason as wct');
         $this->db->join('aa_bank_reason wcntry','wct.quality_id=wcntry.quality_id','left');
        ...
        if (@$requestData['order'][0]['column'] && @$requestData['order'][0]['dir']) {
            ...
            $this->db->order_by("wct.quality_id", "desc");
        }
    ...
    function check_preexistance($id, $city_name) {
        ...
        $query = $this->db->get('aa_bank_reason');
