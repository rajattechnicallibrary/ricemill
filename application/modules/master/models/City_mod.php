...
//THIS FUNCTION ADD COLOR1
    public function add($data) {               
        if ($this->db->insert("am_city", $data)) {
            return true;
        }
    }
...
//  THIS FUNCTION EDIT city DATA
    function edit($id) {
    //    pr($_POST); die;
        $data['state_id'] = $this->input->post('state_name');
...
//  THIS FUNCTION VIEW city DATA
    function view($id) {
        $this->db->select('as.name as state_name, am_city.name as city_name, am_city.status, as.state_id');
        $this->db->join('am_state as as','as.state_id = am_city.state_id','left');
        $this->db->where('city_id', $id);
        return $query = $this->db->get("am_city")->row();
    }
    ...
    function count_city_data() {
        ...
        return $query = $this->db->get('am_city');
    }
    ...
    function get_city_data($parent_id = "") {        
        ...
        $this->db->select('wct.city_id,wct.name as city_name,wct.status, wcntry.name as country_name,wcntry.state_id');
        $this->db->from('am_city as wct');
        ...
        if (@$requestData['order'][0]['column'] && @$requestData['order'][0]['dir']) {
            ...
            $this->db->order_by("wct.city_id", "desc");
        }
